...
from lists.list import (answer_yes_list, answer_YES_list1, answer_YES_list2, answer_NO_list1, answer_NO_list2,
                        ...
                        answer_pnh_list, answer_pes_list, stop_list)
...
@dp.message(Text(text='погода', ignore_case=True), StateFilter(default_state))                      #сработает на приветствие
...
@dp.message(Text(text='время', ignore_case=True), StateFilter(default_state))                #сработает на приветствие
...
async def process_cancel_command_state(message: Message, state: FSMContext):
    ...
    await state.clear()                                                                         # Сбрасываем состояние                                                                       # Сбрасываем состояние
...
async def process_start_command(message: Message):
    ...
    await state.set_state(FSMFillForm.first) 
...
# Этот хэндлер будет срабатывать на любые сообщения, 
# которые отправляются из состояний
