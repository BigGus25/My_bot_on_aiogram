...
from lists.list import (answer_yes_list, answer_YES_list1, answer_YES_list2, answer_NO_list1, answer_NO_list2,
                        ...
                        answer_pnh_list, answer_pes_list, stop_list, Good_list, answer_krys_list, anketa_list)
...
async def warning_not_name(message: Message):
    await message.answer(text='То, что вы отправили не похоже на имя.\n'
                              ...
                              'Если хочешь прервать заполнение анкеты - '
...
async def process_age_sent(message: Message, state: FSMContext):
    ...
    await message.answer(text='Спасибо! А теперь загрузи, пожалуйста, своё фото')
...
async def warning_not_age(message: Message):
    await message.answer(
        text='Возраст должен быть целым числом от 1 до 120\n\n'
             'Попробуй еще раз\n\nЕсли хочешь прервать '
             'заполнение анкеты - отправь команду /cancel')
...
#async def warning_not_photo(message: Message, state: FSMContext):
    answer = message.text.lower();
    if answer in ["без фото", "не хочу отправлять", "не хочу отправлять фото", "не отправлю", "давай без фото", "не хочу", "не буду", "да ну", "не загружу", "хм"]:
        ...
        await message.answer(text='Пожалуйста, на этом шаге отправьте ваше фото\n')
# 06/07/2024 ___________________ФОТО_______________________________________________________________________________________________       
